...
class ExpensesScreen extends StatelessWidget {
  ...
  Widget _header(BuildContext ctx, ExpenseProvider ep) => Padding(
    ...
    child: Row(children: [
      ...
      const Spacer(),
      Column(children: [
        const Text('المصروفات', style: TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.bold)),
        const SizedBox(height: 4),
        Container(
          padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 5),
          decoration: BoxDecoration(color: Colors.cyan.shade700, borderRadius: BorderRadius.circular(20)),
          child: Text('الإجمالي: ${ep.totalAmount.toStringAsFixed(0)} ر.ي', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 11))),
      ]),
      const Spacer(),
  ...
  Widget _filterBtn(BuildContext ctx, ExpenseProvider ep) => Align(
    alignment: Alignment.centerLeft,
    child: Padding(padding: const EdgeInsets.symmetric(horizontal: 16),
      child: _circleBtn(Icons.tune, onTap: () => _showFilter(ctx, ep))));
  ...
  Widget _card(BuildContext ctx, ExpenseModel e, ExpenseProvider ep) {
    ...
    return Dismissible(
      ...
      background: Container(margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.only(right: 20), decoration: BoxDecoration(color: Colors.red.withOpacity(0.2), borderRadius: BorderRadius.circular(16)),
        alignment: Alignment.centerRight, child: const Icon(Icons.delete, color: Colors.redAccent)),
      ...
      child: Container(
        ...
        child: Row(children: [
          IconButton(icon: const Icon(Icons.delete_outline, color: Colors.redAccent), onPressed: () => ep.deleteExpense(e.id!)),
          const Spacer(),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text('${e.amount.toStringAsFixed(0)} ر.ي', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 4),
            Text('النوع: ${e.category}', style: const TextStyle(color: Colors.grey, fontSize: 12)),
            Text(e.createdAt.substring(0, 10), style: const TextStyle(color: Colors.grey, fontSize: 11)),
            if (e.notes != null && e.notes!.isNotEmpty) Text('ملاحظة: ${e.notes}', style: const TextStyle(color: Colors.grey, fontSize: 11)),
          ]),
  ...
  Widget _empty() => const Expanded(child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
    Icon(Icons.money_off, color: Colors.white24, size: 80),
    SizedBox(height: 12),
    Text('لا توجد مصروفات', style: TextStyle(color: Colors.white24, fontSize: 18, fontWeight: FontWeight.bold)),
    Text('اضغط + لإضافة مصروف', style: TextStyle(color: Colors.white24, fontSize: 13)),
  ])));
  ...
  void _showAdd(BuildContext ctx, ExpenseProvider ep) => showModalBottomSheet(
    ...
    builder: (_) => _AddExpenseSheet(ep: ep, onSave: () => ctx.read<HomeProvider>().refresh()));
...
class _AddExpenseSheetState extends State<_AddExpenseSheet> {
  ...
  Widget build(BuildContext ctx) => Container(
    ...
    child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.end, children: [
      ...
      const Center(child: Text('إضافة مصروف', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold))),
      ...
      Container(
        ...
        child: DropdownButtonHideUnderline(child: DropdownButton<String>(
          ...
          items: ExpenseModel.categories.where((c) => c != 'الكل').map((c) =>
            DropdownMenuItem(value: c, child: Align(alignment: Alignment.centerRight, child: Text(c)))).toList(),
        ))),
      const SizedBox(height: 12),
      ...
      Row(children: [
        ...
        Expanded(flex: 2, child: ElevatedButton(
          onPressed: _saving ? null : () async {
            ...
            final ok = await widget.ep.addExpense(amt, _category, notes: _notesCtrl.text.isEmpty ? null : _notesCtrl.text);
            widget.onSave?.call();
...
class _FilterSheetState extends State<_FilterSheet> {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.end, children: [
      Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: Colors.white24, borderRadius: BorderRadius.circular(2)))),
      const SizedBox(height: 16),
      const Center(child: Text('تصفية المصروفات', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold))),
      const SizedBox(height: 20),
      const Text('حسب التصنيف', style: TextStyle(color: Colors.grey)),
      const SizedBox(height: 10),
      Wrap(spacing: 8, runSpacing: 8, alignment: WrapAlignment.end,
        children: ExpenseModel.categories.map((c) => GestureDetector(
          onTap: () => setState(() => _cat = c),
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
            decoration: BoxDecoration(color: _cat == c ? Colors.cyan.shade600 : Colors.transparent, borderRadius: BorderRadius.circular(8), border: Border.all(color: _cat == c ? Colors.cyan.shade600 : Colors.white24)),
            child: Text(c, style: TextStyle(color: Colors.white, fontWeight: _cat == c ? FontWeight.bold : FontWeight.normal))),
        )).toList()),
      const SizedBox(height: 24),
      SizedBox(width: double.infinity, child: ElevatedButton(
        onPressed: () { widget.ep.filter(category: _cat); Navigator.pop(context); },
        style: ElevatedButton.styleFrom(backgroundColor: Colors.cyan, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
  ...
  Widget build(BuildContext context) => SizedBox(
    ...
    child: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
      Row(children: [
        ...
        const Spacer(),
        const Text('تقرير المصروفات', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
      ]),
      const SizedBox(height: 16),
      Container(color: const Color(0xFF162529), padding: const EdgeInsets.symmetric(vertical: 10),
      ...
      Expanded(child: ListView.builder(
        ...
        itemBuilder: (_, i) {
          ...
          return Container(padding: const EdgeInsets.symmetric(vertical: 12),
            ...
            child: Row(children: [
              Expanded(child: Center(child: Text(e.createdAt.length > 18 ? e.createdAt.substring(11, 16) : '-', style: const TextStyle(color: Colors.white, fontSize: 11)))),
              Expanded(child: Center(child: Text(e.createdAt.substring(0, 10), style: const TextStyle(color: Colors.white, fontSize: 11)))),
              Expanded(child: Center(child: Text('${e.amount.toStringAsFixed(0)} ر.ي', style: const TextStyle(color: Colors.white, fontSize: 11)))),
              Expanded(child: Center(child: Text(e.category, style: const TextStyle(color: Colors.white, fontSize: 11)))),
            ]));
        })),
      const Divider(color: Colors.white10),
      Align(alignment: Alignment.centerLeft, child: Text('الإجمالي: ${ep.totalAmount.toStringAsFixed(0)} ر.ي', style: const TextStyle(color: Colors.white70, fontWeight: FontWeight.bold))),
    ])));
}
